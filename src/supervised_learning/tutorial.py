...
from src.unsupervised_learning.knn_learner import KNNClusterer

df = pd.read_csv('../../dataset/dishes_df.csv')

knn_clusterer = KNNClusterer(df, n_clusters=10, random_state=42)
df_with_clusters = knn_clusterer.clusterize()

linear_regressor = LinearRegressor(df_with_clusters)
linear_regressor_no_knn = LinearRegressor(df)

ridge_regressor = RidgeRegressor(df_with_clusters, alpha=0.1, k=5)

lasso_regressor = LassoRegressor(df_with_clusters, alpha=5, k=5)
...
pred2 = linear_regressor_no_knn.predict(new_data.drop('cluster', axis=1))
